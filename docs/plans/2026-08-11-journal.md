...
## Addendum du 2026-08-11 — deux retours d'usage

Écrit après que l'utilisateur a essayé la section : la largeur du volet n'est pas
retenue, et il veut lire ses notes en Markdown rendu tant qu'il n'a pas cliqué
dedans. La tâche 10 (README) passe **après** ces deux-là, pour qu'elle décrive ce
qui existe réellement.

---

### Task 11: La largeur du volet, retenue

**Files:**
- Modify: `Cairn/App/SplitViewHoldingPriorities.swift`
- Test: `Tests/DetailPaneWidthTests.swift` (ajouts, si la correction dégage une valeur testable)

**Symptôme rapporté :** dans le journal, le volet de droite revient à sa largeur
minimale au lieu de garder celle qu'on lui a donnée.

**Hypothèse à confirmer avant toute correction.** La largeur mémorisée n'est
réappliquée que via `pendingWidth`, et `pendingWidth` n'est renseigné qu'à deux
endroits : `restoreOnLaunch` et `handOver(to:)`. `handOver` ne fait quelque chose
que si le `paneKind` **change**, c'est-à-dire à un changement de section.

Or le volet du journal se ferme et se rouvre sans jamais changer de section :
`RootView.detailColumn` rend `collapsedDetailColumn` dès que
`journalSelection == nil`, et la liste du journal **ne sélectionne rien à
l'arrivée**. Donc :

1. on entre dans le journal, colonne fermée → `handOver(.journal)` pose
   `pendingWidth`, `applyPendingWidth` voit une largeur de 0 et repart sans
   l'appliquer (en le gardant) ;
2. on sélectionne une note → la colonne se rouvre → l'observateur de
   redimensionnement applique enfin la largeur. **Ce premier passage marche.**
3. on désélectionne (échap, `h`, suppression) → colonne à 0 ;
4. on resélectionne → la colonne se rouvre, mais `pendingWidth` est nil depuis
   l'étape 2 et rien ne le repose. AppKit rend la largeur qu'il veut.

La liste des activités masque le problème parce qu'elle sélectionne une ligne
d'office à l'arrivée : sa colonne reste ouverte, et le cas 3-4 y demande une
désélection explicite.

**Confirmer d'abord**, par lecture et par un test si possible, que c'est bien ce
chemin — ne pas corriger sur la foi de ce paragraphe.

**Direction de correction**, si l'hypothèse tient : la réouverture de la colonne
doit redemander la largeur du `kind` courant, pas seulement le changement de
section. Dans `observeDetailWidth`, retenir la largeur précédente et, quand elle
passe de 0 à non-nul, poser `pendingWidth = DetailPaneWidth.saved(for: kind)`
avant d'appeler `applyPendingWidth`. La transition 0 → ouvert est une valeur
pure — extraire la décision (`shouldRestore(previousWidth:newWidth:)`) et la
tester plutôt que de la laisser dans la fermeture de l'observateur, comme le
fait déjà `DetailPaneWidth.dividerPosition`.

Attention à ne pas réintroduire ce que les commentaires du fichier documentent
déjà comme des pièges : seul un vrai glissement (`NSSplitViewUserResizeKey`)
enregistre une largeur, et une largeur sous le plancher du `kind` est une
fermeture, pas une mesure.

- [ ] **Step 1: Confirmer l'hypothèse**, en lisant le chemin et en écrivant le
      test qui échoue si la transition 0 → ouvert ne redemande rien.
- [ ] **Step 2: Corriger**, en extrayant la décision en valeur testée.
- [ ] **Step 3: Build sans avertissement + suite complète.**
- [ ] **Step 4: Commit** — `fix(journal): le volet retrouve sa largeur en rouvrant`

---

### Task 12: Le rendu Markdown tant qu'on n'écrit pas

**Files:**
- Modify: `Cairn/Features/Journal/JournalDetailView.swift`
- Modify: `Cairn/Features/Shared/MarkdownBlock.swift` (si le frontmatter doit être sauté)
- Test: `Tests/MarkdownTests.swift` (ajouts)

**Ce qui change.** Le volet affiche la note **rendue** — titres, listes, citations,
via le `MarkdownText` qui sert déjà les notes d'activité — et bascule sur le champ
de texte dès qu'on clique dedans, ou sur `e`, `n` ou `⏎` depuis la liste. Échap
sort de l'éditeur et rend la note à nouveau, comme aujourd'hui.

C'est un revirement assumé de la décision « toujours éditable, texte brut » du
spec : l'utilisateur a essayé et préfère lire en rendu. Mettre le spec à jour.

**Décidé avec l'utilisateur :** en passant du rendu à l'éditeur, **le curseur va
en fin de texte** et non à l'endroit cliqué. Ce sont deux vues distinctes et
SwiftUI ne transporte pas la position du clic de l'une à l'autre. Accepté.

**Points à traiter, qui ne sont pas dans le composant existant :**

- **Le frontmatter.** `MarkdownParser` n'a pas de règle pour `---`, qui
  ressortirait donc en paragraphe « --- » au-dessus de chaque note qui en porte.
  Le sauter, comme `JournalNote.summary` le fait déjà, et tester ce saut.
- **Une note vide** n'a rien à afficher, donc rien à cliquer. Prévoir une
  invite discrète (« Cliquez pour écrire ») et rendre toute la surface du volet
  cliquable, pas seulement le texte.
- **Le retour au rendu** doit se faire aussi en changeant de note : ouvrir une
  note ne doit jamais arriver en mode édition sans qu'on l'ait demandé.
- **Une note illisible** garde son `ContentUnavailableView` et ne devient
  jamais éditable.
- Le bandeau de conflit et la ligne d'échec d'écriture restent visibles dans
  les deux modes — ce sont eux qui préviennent d'une perte.

Ne pas toucher à l'enregistrement : c'est toujours `onEdit` → `store.update`,
avec la même temporisation.

- [ ] **Step 1: Tests d'abord** pour le saut de frontmatter dans le parseur.
- [ ] **Step 2: Le mode lecture** dans `JournalDetailView`.
- [ ] **Step 3: La bascule** au clic et aux touches, et le retour au rendu.
- [ ] **Step 4: Build sans avertissement + suite complète.**
- [ ] **Step 5: Commit** — `feat(journal): la note se lit en Markdown rendu, s'édite au clic`
